        ...
        function ITERATIVE-DEEPENING-SEARCH(problem) returns a solution, or failure
            for depth = 0 to inf 
                ...
                if result != cutoff 
                    return result
        """
        while( (best_score != float("-inf")) or (best_score != float("inf") ) ):
            try:
                best_move, best_score = self.alphabeta_with_score( game, search_depth, float("-inf"), float("inf") )

            except SearchTimeout:
                print("search timeout- best move is (w/ score)", best_move, best_score )
                return best_move  # Handle any actions required after timeout as needed

            #print ("dep, score=", search_depth, best_score )
            #if( search_depth > 3 ):
            #    return best_move

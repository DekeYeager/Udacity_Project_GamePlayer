...
class IsolationPlayer:
    ...
    ********************  DO NOT MODIFY THIS CLASS  ********************
    ...
    timeout : float (optional)
        ...
        timer expires.
    """
    #TBD ORIGINAL def __init__(self, search_depth=3, score_fn=custom_score, timeout=10.):
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        try:
            # The try/except block will automatically catch the exception
            # raised when the timer is about to expire.alpha=float("-inf"), beta=float("inf")
            #nneded to pass uda assistant
    ...
    def alphabeta_value(self, game, depth, alpha, beta):
        ...
        #print(self.pr_tabs(depth), "DEPTH=", depth)

        if depth == 0:
            #return self.score(game, game.active_player)
            #print(self.pr_tabs(depth), game.to_string() )
            return self.score(game, self)
        ...
        #max player
        if (game.active_player == self):
            #print(self.pr_tabs(depth), "A,B, Legal Moves(max): ",alpha, beta, legal_moves)
            final_score = float("-inf")
            for m in legal_moves:
                new_game = game.forecast_move(m) #create new board with change
                #print(self.pr_tabs(depth), new_game.to_string() )
                score = self.alphabeta_value(new_game, depth-1, alpha, beta)
                #print(self.pr_tabs(depth), "move, score", m, score)
                if score > final_score:
                    final_score = score
                if final_score >= beta:
                    #print(self.pr_tabs(depth), "(BETA CUTOFF) Pruning because B<A", beta, alpha )
                    return final_score
        ...
        #min player
            ...
            final_score = float("inf")
            for m in legal_moves:
                #print(self.pr_tabs(depth), "A,B, Legal Moves(min): ",alpha, beta, legal_moves)
                new_game = game.forecast_move(m) #create new board with change
                #print(self.pr_tabs(depth), new_game.to_string() )
                score = self.alphabeta_value(new_game, depth-1, alpha, beta)
                #print(self.pr_tabs(depth), "move, score", m, score)
                if score < final_score:
                    final_score = score
                if final_score <= alpha:
                    #print(self.pr_tabs(depth), "(ALPHA CUTOFF) Pruning because B<A", beta, alpha )
                    return final_score
                if final_score < beta:
                    beta = final_score

        #print(self.pr_tabs(depth), "returning: final score, alpha, beta = ", final_score, alpha, beta )
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        legal_moves = game.get_legal_moves(game.active_player)
        #print ("ROOT:", legal_moves)

        if not legal_moves:
            #print ("ROOT: No legal moves, we are about to lose!")
            #print("\nBoard:\n{}".format(game.to_string()))
            return (-1, -1)
        ...
        if (game.active_player == self):
            ...
            for m in legal_moves:
                new_game = game.forecast_move(m) #create new board with change
                score = self.alphabeta_value(new_game, depth-1, alpha, beta)
                #print( "ROOTMAX:", m, " has Score ", score);
        ...
        #min player ---I dont believe this will ever get called!
            ...
            for m in legal_moves:
                ...
                new_game = game.forecast_move(m) #create new board with change
                score = self.alphabeta_value(new_game, depth-1, alpha, beta)
                #print( "ROOTMIN:", m, " has Score ", score);
                if score < Chosen_Score:
                    Chosen_Score = score
                    Chosen_Move = m
                if Chosen_Score <= alpha:
        ...
        #print("ROOT: chosen, alpha, beta = ", Chosen_Move, alpha, beta )
        return Chosen_Move
